...
export default function SettingsPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (trimmedPhoneNumber && !PHONE_PATTERN.test(trimmedPhoneNumber)) {
      ...
      <div className="max-w-full">
        <header className="border-b border-border pb-7">
          ...
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-brand-ash">
            Manage your profile and the contact details used for neighbourhood
            activity.
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-2 text-xs text-brand-ash/70">
                  This email is managed through your account authentication.
                </p>
              </div>
              <div>
                <label
          ...
          <section className="border-b border-border py-7">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-md border border-border bg-brand-abyss px-4 py-3">
                <p className="text-xs text-brand-ash/70">System role</p>
          ...
          <section className="pt-6">
            <div className="flex justify-end">
              <Link
                href="/dashboard"
                className="mr-3 inline-flex h-9 items-center justify-center rounded-md border border-border px-3.5 text-sm font-medium text-brand-ash transition-colors hover:bg-brand-slate hover:text-brand-frost"
              >
                Cancel
              </Link>
              <button

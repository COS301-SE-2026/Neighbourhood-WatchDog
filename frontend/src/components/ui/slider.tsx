...
function Slider({
  ...
  let resolvedValues: number[];
  if (Array.isArray(value)) {
    resolvedValues = value;
  }
  else if (Array.isArray(defaultValue)) {
    resolvedValues = defaultValue;
  }
  else {
    resolvedValues = [min, max];
  }


  const _values = React.useMemo(() => resolvedValues, [resolvedValues]);
    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="relative block size-3 shrink-0 rounded-full border border-ring bg-brand-depth ring-ring/50 transition-[color,box-shadow] select-none after:absolute after:-inset-2 hover:ring-3 focus-visible:ring-3 focus-visible:outline-hidden active:ring-3 disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )

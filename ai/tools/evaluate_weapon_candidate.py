...
DEFAULT_MODEL = AI_ROOT / "data/curated/weapons-gun-knife-v1/training-runs/weapons-gun-knife-smoke-v1/weights/best.pt"
DEFAULT_REPORT = AI_ROOT / "evaluation/reports/weapon-candidate-smoke-v1"
...
def main() -> None:
    ...
    lines = ["# Weapon Detection Candidate Smoke Test v1", "", f"- Evaluation images: **{len(images)}**", f"- Current model: `{report['model']['path']}`", f"- Confidence / NMS / match IoU: **{args.confidence} / {args.nms_iou} / {args.match_iou}**", "", "| Class | TP | FP | FN | Precision | Recall | F1 |", "|---|---:|---:|---:|---:|---:|---:|"]

    for name, value in metrics.items():
        lines.append(f"| {name} | {value['tp']} | {value['fp']} | {value['fn']} | {value['precision']:.4f} | {value['recall']:.4f} | {value['f1']:.4f} |")

    lines += ["", "## Scope", "", "The candidate model uses class 0 for Gun and class 1 for knife. Both outputs are scored against the frozen two-class evaluation set.", "", "## Limitations", *[f"- {item}" for item in report["limitations"]]]
    ...
    print(f"Weapon candidate smoke-test report: {args.report_dir}")

...
> **All commands in this section must be run from the `ai` directory.** Navigate there first:
> ```
> cd ai
> ```
...
Once the virtual environment is activated, you are ready to install the project dependencies. Dependencies are the external packages the AI pipeline relies on, such as PyTorch, YOLOv8, OpenCV, and DeepSORT. They are listed in `requirements.txt` and installed using `pip`, Python's package installer.

> **Note:** This installation is large. PyTorch and its CUDA dependencies are several hundred megabytes. Ensure you have a stable internet connection and sufficient disk space (at least 4GB free) before proceeding.
...
The `--timeout 300` flag gives pip more time per download, which is recommended given the size of the packages.
